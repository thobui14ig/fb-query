...
export class GetCommentPublicUseCase {
    ...
    async getCmtPublic(postId: string, isCheckInfoLink: boolean = false): Promise<IGetCmtPublicResponse | null> {
        ...
        try {
            ...
            if (!dataComment) {
                ...
                dataComment = await this.getCommentWithCHRONOLOGICAL_UNFILTERED_INTENT_V1(postId, proxy)
            }
    ...
    async handleDataComment(response) {
        ...
        userIdComment = (isNumeric(userIdComment) ? userIdComment : (await this.getUuidPublic(comment?.author.id)) || userIdComment)
    ...
    async getCommentWithCHRONOLOGICAL_UNFILTERED_INTENT_V1(postId: string, proxy: ProxyEntity) {
        const httpsAgent = getHttpAgent(proxy)
